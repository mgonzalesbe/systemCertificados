...
def _ensure_usuarios_columns(cursor):
    ...
    if not _column_exists(cursor, "Usuarios", "DocumentoIdentidad"):
        cursor.execute("ALTER TABLE Usuarios ADD DocumentoIdentidad NVARCHAR(32) NULL")
    if not _column_exists(cursor, "Usuarios", "Nombres"):
        cursor.execute("ALTER TABLE Usuarios ADD Nombres NVARCHAR(100) NULL")
    if not _column_exists(cursor, "Usuarios", "Apellidos"):
        cursor.execute("ALTER TABLE Usuarios ADD Apellidos NVARCHAR(100) NULL")
    if not _column_exists(cursor, "Usuarios", "Universidad"):
        cursor.execute("ALTER TABLE Usuarios ADD Universidad NVARCHAR(200) NULL")
    if not _column_exists(cursor, "Usuarios", "Area"):
        cursor.execute("ALTER TABLE Usuarios ADD Area NVARCHAR(150) NULL")
